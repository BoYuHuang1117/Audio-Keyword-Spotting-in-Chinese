...
Ty = 247 # The number of time steps in the output of our model
...
# BGN section
BGN_Cafe_tmp = wavfile.read("./data/Cafeteria_10s_add7db.wav")
BGN_Pub_tmp = wavfile.read("./data/Pub_10s_add7db.wav")
BGN_Whitenoise_tmp = wavfile.read("./data/wn_10s_add7db.wav")
BGN_Cafe = np.copy(BGN_Cafe_tmp)
BGN_Pub = np.copy(BGN_Pub_tmp)
BGN_whitenoise_copy = np.copy(BGN_Whitenoise_tmp)
...
def get_random_time_segment(segment_ms):
    ...
    segment_start = np.random.randint(low=0, high=480000-segment_ms)  
...
def insert_audio_clip_neg(background, audio_clip, previous_segments):
    # the negative label cut into half length
    segment_ms = len(audio_clip[1])/2
    segment_time = get_random_time_segment(segment_ms)
...
def insert_ones(y, segment_end_ms):
    ...
    # change the segment into ms scale
    segment_end_ms = (segment_end_ms/48000)*1000
    # duration of the background (in terms of spectrogram time-steps)
    segment_end_y = int(segment_end_ms * Ty / 10000.0)
    
    # Add 1 to the y array for 15 time segment
    for i in range(segment_end_y+1, segment_end_y+16):
        if i < Ty:
            y[0, i] = 1.0
...
def create_training_example(background, activates, negatives, person, track):
    ...
    for random_activate in random_activates:
        ...
        segment_start, segment_end = segment_time
        # for activate wrod, insert 1 into label array
        y = insert_ones(y, segment_end)
        
    # Select random negatives audio recordings from the entire list of "negatives" recordings
    ...
    for random_negative in random_negatives:
        background_new, _ = insert_audio_clip_neg(background_new, random_negative, previous_segments)
    ...
    x = MFCC_feature_extraction_psf(str(person)+"_"+str(track)+"_train.wav")
...
def MFCC_feature_extraction_psf(wav_file):
    ...
    mfccs = psf.mfcc(sig, samplerate=fs, numcep=13, nfft=1200, appendEnergy=False) 
    fig, ax = plt.subplots()
    mfccs_data = np.swapaxes(mfccs, 0 ,1) 
...
for i in range(1, 73):
    for j in range(1, 11):        
        ...
        #bgn_random=np.random.randint(10)%3
        bgn_random=0
        if bgn_random==0:
            x_mix, y_mix=create_training_example(BGN_Pub_copy, dic_person_label1[i], dic_person_label0[i], i, j)
        if bgn_random==1:
            x_mix, y_mix=create_training_example(BGN_Cafe_copy, dic_person_label1[i], dic_person_label0[i], i, j)
        if bgn_random==2:
            x_mix, y_mix=create_training_example(BGN_whitenoise_copy, dic_person_label1[i], dic_person_label0[i], i, j)
        ...
        BGN_Cafe_tmp = wavfile.read("./data/Cafeteria_10s_add7db.wav")
        BGN_Cafe = np.copy(BGN_Cafe_tmp)
        tmp3 = np.copy(np.array(BGN_Cafe_tmp[1], dtype=np.float32))
        tmp4 = np.round(32767*tmp3)
        BGN_Cafe[1] = tmp4.astype(np.int16)
        BGN_Cafe[1] = BGN_Cafe[1]
        BGN_Cafe_copy=BGN_Cafe.copy()
